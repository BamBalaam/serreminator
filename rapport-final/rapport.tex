...
Cet état de l'art est séparé en deux parties : la première détaille les différentes méthodes de régulation automatique et la seconde étudie les différentes méthodes pour configurer l'algorithme PID (qui est détaillé en \ref{PID}).
...
Nous étudierons premièrement ``BangBang'', l'algorithme le plus simpliste, utilisé dans les thermostats.
Ensuite nous étudierons PID ainsi que ses variantes.
Pour finir nous survolerons MPC un algorithme prédicitf bien plus avancé.
